...
;var JotForm = {
    ...
    init: function(callback) {
        var ready = function() {
            try {
                ...
                if (typeof $$('.jotform-form') !== 'undefined' && typeof $$('.jotform-form')[0] !== 'undefined' && typeof $$('.jotform-form')[0].reset === 'function' && typeof $$('.jotform-form')[0].autocomplete !== 'undefined' && $$('.jotform-form')[0].autocomplete == 'off') {
    ...
    captureMultipleUploadsBasicMarkup: function(fileInput) {
        if (!JotForm.rawMultipleFileInputs)
            ...
            xhr.addEventListener('load', function() {
                ...
                if (200 === response.status) {
                    if (options.onSuccessCb)
                        options.onSuccessCb(response);
                } else {
                    if (options.onFailureCb)
                        options.onFailureCb();
                }
                if (options.onCompleteCb)
                    options.onCompleteCb();
        ...
        if ((JotForm.newPaymentUI) && window.paymentType === 'product' && window.FORM_MODE !== 'cardform') {
            $$('.form-product-item').each(function(element) {
                element.observe('click', function(event) {
                    if (event.target.type !== 'text' && event.target.type !== 'select-one' && event.target.type !== 'checkbox' && event.target.tagName !== 'LABEL' && !event.target.hasClassName('image_zoom')) {
                ...
                element.observe('change', function() {
                    ...
                    $$('.form-product-item .form-radio').each(function(subElement) {
                        if (subElement !== element)
                            subElement.up('.form-product-item').classList.remove('p_selected')
                    });
                });
            });
        }
            ...
            $('id_' + field).select(selector).each(function(el) {
                if (['radio', 'checkbox'].indexOf(el.type) > -1) {
                    ...
                    if (el.parentNode.dataset.type === 'signaturebox') {
                        ...
                        if (signatureImage)
                            signatureImage.setAttribute('src', '');
                    }
                    el.value = (el.id in JotForm.defaultValues) ? JotForm.defaultValues[el.id] : "";
                }
            });
    ...
    requireField: function(qid, req, additionalRequireType) {
        ...
        elements.each(function(el) {
            ...
            }) || el.hasClassName('jfDropdown-search') || el.hasClassName('jfRating-shortcut-input') || el.hasClassName('__PrivateStripeElement-input')) {
                return;
            }
            var validations = [];
        ...
        try {
            ...
            elements.each(function(input) {
                ...
                case 'SELECT':
                    ...
                    break;
                case 'INPUT':
                    if ((['checkbox', 'radio'].include(input.type) && !input.checked) || input.type === 'file' || (['year_' + qid, 'month_' + qid, 'day_' + qid, 'lite_mode_' + qid]).include(input.id)) {
                        input.disable();
                    }
                default:
    ...
    setCalculationEvents: function() {
        ...
        $A(JotForm.calculations).each(function(calc, index) {
            if (!calc.operands || typeof calc.operands === 'function')
                return;
            ...
            for (var i = 0; i < ops.length; i++) {
                ...
                if (!opField || opField.empty() || (!$('id_' + opField) && !$(mixedOpField)))
                    continue;
    ...
    checkCalculation: function(calc) {
        ...
        if (!$('id_' + result) && !$(mixedResult))
            return;
        try {
            if (!['text', 'email', 'textarea', 'calculation', 'combined', 'address', 'datetime', 'time', 'html', 'authnet', 'paypalpro', 'number', 'radio', 'checkbox', 'select', 'matrix', 'widget', 'signature', 'braintree', 'stripe', 'square', 'eway', 'bluepay', 'firstdata', 'chargify', 'echeck', 'payu', 'pagseguro', 'moneris', 'paypal', 'dwolla', 'bluesnap', 'paymentwall', 'payment', 'paypalexpress', 'payjunction', '2co', 'cardconnect', 'clickbank', 'onebip', 'worldpay', 'rating', 'hidden', 'file', 'other', 'mixed', 'sofort', 'payoneer', 'paysafe', 'gocardless', 'stripeACH', 'paypalSPB', 'cybersource', "paypalcomplete", 'inline'].include(JotForm.calculationType(result)))
                return;
        } catch (e) {
            console.log(e);
        }
        ...
        var getValue = function(data, numeric) {
            ...
            case 'select':
                var optionValue = function(option) {
                    if (JotForm.newDefaultTheme && option.value === '')
                        return '';
                ...
                try {
                    if ((typeof output == "number" && output > 0) || (typeof output == "string" && output.replace(/\s/g, "").length > 0 && output !== "0")) {
                        ...
                        if ($('input_' + result)) {
                            ...
                            if (!isNaN(year))
                                $$('#cid_' + result + ' .jfField[data-type="year"] input')[0].value = year;
                            if (!isNaN(month))
                                $$('#cid_' + result + ' .jfField[data-type="month"] input')[0].value = month;
                            if (!isNaN(day))
                                $$('#cid_' + result + ' .jfField[data-type="day"] input')[0].value = day;
                        } else {
                            if (!isNaN(year))
    ...
    initTestEnvForNewDefaultTheme: function() {
        if (window && window.location && window.location.search) {
            ...
            if (params.indexOf('ndtTestEnv=1') > -1) {
                ...
                if (formSection)
                    formSection.classList.add('ndt-test-env');
            }
        }
    },
    ...
    getMessageFormTermsAndCondition: function() {
        if (window.location.href.indexOf('ndt=1') === -1)
            return;
    ...
    errored: function(input, message) {
        ...
        }).insert('<img src="' + preLink + 'images/exclamation-octagon.png"> ' + message).insert(new Element('div',{
    ...
    disableSubmitForCard: function(action, isConditionMatch) {
        if (window.FORM_MODE !== 'cardform' || !window.CardForm || !window.CardForm.cards)
            return;
    ...
    toggleDisableSubmitMessage: function() {
        ...
        if (typeof (submitButtonCardIndex) === "number" && document.getElementsByClassName('jfCard')) {
            ...
            if (!jfCardElement || !disableSubmitErrorMessageElement)
                return;
            ...
            if (vals.include("required")) {
                ...
                if (input.tagName == 'INPUT' && input.readAttribute('type') == "file") {
                    ...
                    var hasOneAnswerEveryRows = Array.from(rows).map(getElementsInRow).filter(function notEmpty(el) {
                        return el.length;
                    }).map(anyRowElementsFilled).every();
                    if (vals.include("requireEveryRow") && !hasOneAnswerEveryRows) {
                        ...
                        if (JotForm.newDefaultTheme) {
                            ...
                            for (var i = 0; i < allChildren.length; i++) {
                                if ((allChildren[i].getAttribute('type') === 'radio' || allChildren[i].getAttribute('type') === 'checkbox') && allChildren[i].checked) {
                                    return false;
                                }
                                if (allChildren[i].getAttribute('type') !== 'radio' && allChildren[i].getAttribute('type') !== 'checkbox' && allChildren[i].value.length > 0) {
                                    return false;
                                }
                            }
                        }
    ...
    setQuestionMasking: function(toSelector, type, maskValue, unmask) {
        if (!maskValue && maskValue === null && !unmask)
            return;
